function [ydata D] = tsne(X, labels, no_dims, initial_dims, perplexity,iter)
%TSNE Performs symmetric t-SNE on dataset X
%
%   mappedX = tsne(X, labels, no_dims, initial_dims, perplexity,iterations)
%   mappedX = tsne(X, labels, initial_solution, perplexity,iterations)
%
...
size(X)
    ...
    %clear D		% Enrico edit, it actually returns D
    
    % Run t-SNE
    if initial_solution
        ydata = tsne_p(P, labels, ydata,iter);
    else
        ydata = tsne_p(P, labels, no_dims,iter);
    end
