...
sensations_classes
load_labels_FI % makes variable labels
...
if(OUTLINEONLY ==1 )
    export_fig -m4 'figs/body_sim_tsne_dbscan_w_contour_outlines_FI.png'
else
    export_fig -m4 'figs/body_sim_tsne_dbscan_w_contour_FI.png'
end
ccc=colorbar;
set(ccc,'Limits',[0 1])
...
if(OUTLINEONLY ==1 )
    export_fig -m4 'figs/body_sim_tsne_dbscan_w_contour_outlines_colorbar_FI.png'
else
    export_fig -m4 'figs/body_sim_tsne_dbscan_w_contour_colorbar_FI.png'
end
...
% else
    export_fig -m4 'figs/body_sim_tsne_on_bodies_FI.png'
%end
error('stop')
...
for x=1:GR
    for y=1:GC
        ...
        for dx=-1:1
            for dy=-1:1
                th=text(y*W*XF+W/2+dx,x*H*YF+H*(1+YF)/2+dy,templabel);
                set(th,'Color',[1 1 1]*.8);
                if(grid_of_bodies_cluster(x,y)>0)
        ...
        dx=0;
        dy=0;
        th=text(y*W*XF+W/2+dx,(GR-x)*H*YF+H*(1+YF)/2+dy,templabel);
        if(dx==0 && dy==0)
            if(grid_of_bodies_cluster(x,y)>0)
                    set(th,'Color', classColors(grid_of_bodies_cluster(x,y),:));
...
export_fig(['figs/body_grid_' num2str(NR) '_dbscan_FI.png'],'-m4')
