...
// WE HAVE THE SUBJECT, LET'S START THE SESSION
 ...
 //   $outtext.="<div id='progress-bar' class='all-rounded' style=\"float:right\"> <div id='progress-bar-percentage' class='all-rounded' style='font-size:14px;font-weight:bold;margin-left:100px;position:absolute'> <span>".$pagetexts['done_perc'].$perc."%<br></span>";
    ...
    if($amount[$currbatchID] == 1){ // if first  batch is completed
        ...
        $outtext.="<br><h1>".$pagetexts['thank-you']."</h1><br><br><br><div style=\"width:400px;text-align:center;margin-left:auto;margin-right:auto\">$somemore<a class=\"bottone\" href=\"http://emotion.becs.aalto.fi/osallistu/\">".$pagetexts['participate']."</a></div>";
		// add an if
    ...
    if($amount[$currbatchID] < 1)
    {
        ...
        if ($type=="form"):
            ...
            $goto=$link."annotate.php?perc=$perc&userID=$userID&presentation=$presentation&currbatchID=$currbatchID";
        endif;
        $helpful['goto'] = $goto;
