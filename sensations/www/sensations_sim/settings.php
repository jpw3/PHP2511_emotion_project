...
//this is the only part you should need to change when setting up a new experiment. Also, read the readme.txt to figure out which textfiles you need to put in $path location.
	...
	$countriesfile = 'countries_FI.txt';
    ...
    $stimuli =      'sensations.txt';        // stimuli to be displayed
    $pgtexts =      'page_texts_FI.txt';   // bits and bobs of text peppered around the system, see example page_texts.txt for explanation what should be found there
    $path="/var/www/emotion.becs.aalto.fi/sensations_sim/";
	$wwwlocation = 'http://emotion.nbe.aalto.fi/sensations_sim/';
...
//these are common for all cases
    ...
    $stimulusfile = 'sensations_FI.txt';
    $stimuliArray = loadTxt($path.$stimulusfile,0);
    $stimulusfile = 'dummy.txt';
